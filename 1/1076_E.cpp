...
void dfs(int node , int par , int h , int sum){

    for(auto &p : q[node]){                 //ADD WHILE ENTERING NODE
        int left = h , right = p.first + h;

        add[left] += p.second;              //This is simple offline range updates
        
        if(right +1  < N)
            add[right+1] -= p.second;
    ...
    // REAL DFS IN THE TREE

    for(auto child : g[node]){
        if(child != par)
            dfs(child , node , h+1 ,sum);
    }

    for(auto &p : q[node]){                 //REMOVE WHILE LEAVING NODE
        int left = h , right = p.first + h;
