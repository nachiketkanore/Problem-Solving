...
vi tree[N];
int broken_node[N];
int subtree_broken[N];

void dfs(int node , int par){

    subtree_broken[node] = broken_node[node];
    ...
    for(auto child : tree[node]){
        ...
        if(child != par){
            ...
            subtree_broken[node] += subtree_broken[child];
...
int32_t main(){
    ...
    for(int x=1;x<=n;x++){
        if(broken_node[x] and (subtree_broken[x] == 1))
            nachiket.pb(x);
    }

    cout << nachiket.size() << endl;
