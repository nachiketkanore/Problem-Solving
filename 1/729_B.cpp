...
int32_t main(){
    ...
    for(int x=1;x<=n;x++){
        for(int y=1;y<=m;y++){
            suff_row[x][y] += suff_row[x][y-1];
        }
    }

    for(int y=1;y<=m;y++){
        for(int x=1;x<=n;x++){
            suff_col[x][y] += suff_col[x-1][y];
        }
    }

    int ans = 0;

    // cout << "SUFF _ ROW" << endl;
    ...
    for(int x=1;x<=n;x++){
        for(int y=1;y<=m;y++){
            if(mat[x][y] == 0){
                int left = suff_row[x][y];
                int up = suff_col[x][y];
                int right = row[x] - left;
                int down = col[y] - up;
                ans += ((left > 0) + (up > 0) + (right > 0) + (down > 0));
            }
        }
    }
