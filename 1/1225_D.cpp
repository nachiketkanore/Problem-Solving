    ...
    for(int x=1;x<=n;x++){
        ...
        for(int i=2;i*i<=a[x];i++){
            if(a[x] % i == 0){
                ...
                if(exponent % k != 0)
                    prime_factorization.pb({prime , exponent % k});

            }
        }

        if(a[x] > 1)
            prime_factorization.pb({a[x] , 1});

        int want = 1;
        int num = 1;

        for(auto &t : prime_factorization){
            for(int i=1;i<=k - t.second;i++)    want *= t.first;
            for(int i=1;i<=t.second ;i++)   num *= t.first;
        }
